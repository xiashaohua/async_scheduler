...
pub type JobToRun  = Box<dyn Send + Sync + FnMut(Uuid, JobsSchedulerLocked) -> Pin<Box<dyn Send + Sync + Future<Output = ()>>>>;


#[async_trait]
pub trait ClientTask:Send {
    async fn run(&self);
}
...
pub trait Job {
    ...
    fn job_id(&self) -> Uuid;
    async fn run(&mut self, jobs: JobScheduler);
...
impl Job for CronJob {
    ...
    async fn run(&mut self, jobs: JobScheduler) {
        (self.run)(self.job_id, jobs).await;
    }
...
impl JobLocked {
    ...
    /// ```
    pub fn new(schedule: &str, run: JobToRun) -> Result<Self, Box<dyn std::error::Error>>
    {
   ...
   /// ```
    pub fn new_cron_job(schedule: &str, run: JobToRun) -> Result<Self, Box<dyn std::error::Error>>
    ...
    /// ```
    pub async fn new_one_shot(duration: Duration, run: JobToRun) -> Result<Self, Box<dyn std::error::Error>>
    ...
    /// ```
    pub async fn new_one_shot_at_instant(instant: std::time::Instant, run: JobToRun) -> Result<Self, Box<dyn std::error::Error>>
    ...
    /// ```
    pub async fn new_repeated(duration: Duration, run: JobToRun) -> Result<Self, Box<dyn std::error::Error>>
