...
pub type Fut =  Future<Output = ()>;
pub type JobToRun<Fut: Future<Output = ()>> =  dyn Send + Sync + FnMut(Uuid, JobsSchedulerLocked) -> Fut ;
...
struct CronJob {
    ...
    pub run: Box<dyn Send + Sync + FnMut(Uuid, JobsSchedulerLocked) -> Future<Output = ()>>,
...
impl JobLocked {
    ...
    /// ```
    pub fn new<T, Fut>(schedule: &str, run: T) -> Result<Self, Box<dyn std::error::Error>>
    where
        T: 'static,
        T:  Send + Sync + FnMut(Uuid, JobsSchedulerLocked) -> Fut,
        Fut: Future<Output = ()>,
    {
   ...
   /// ```
    pub fn new_cron_job<T, Fut>(schedule: &str, run: T) -> Result<Self, Box<dyn std::error::Error>>
    where
        T: 'static,
        T:  Send + Sync + FnMut(Uuid, JobsSchedulerLocked) ->  Fut,
        Fut: Future<Output = ()>,
    ...
    /// ```
    pub fn new_one_shot<T, Fut>(duration: Duration, run: T) -> Result<Self, Box<dyn std::error::Error>>
    where
        T: 'static,
        T:  Send + Sync + FnMut(Uuid, JobsSchedulerLocked) -> Fut,
        Fut: Future<Output = ()>,
    ...
    /// ```
    pub fn new_one_shot_at_instant<T, Fut>(instant: std::time::Instant, run: T) -> Result<Self, Box<dyn std::error::Error>>
    where
        T: 'static,
        T:  Send + Sync + FnMut(Uuid, JobsSchedulerLocked) ->  Fut,
        Fut: Future<Output = ()>,
    ...
    /// ```
    pub fn new_repeated<T, Fut>(duration: Duration, run: T) -> Result<Self, Box<dyn std::error::Error>>
    where
        T: 'static,
        T:  Send + Sync + FnMut(Uuid, JobsSchedulerLocked) ->  Fut,
        Fut: Future<Output = ()>,
